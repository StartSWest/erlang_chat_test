...
There is a second setup I can think of and that is manually starting the applications in both main and server nodes. The last server to start ask for one of the running server to sync the data, also it could be posible to start *pg2* gobal progress group to join the servers so any suscequent messages sent from the users can be send to both servers using *pg2:get_members/1*. I didn't implement this way because of the simplesnes and elegance of the first way which is native erlang distribution.
